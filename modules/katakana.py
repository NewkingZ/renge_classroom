# This page contains katakana specific handling
...
class KatakanaSetup(tk.Frame):
	def __init__(self, root, difficulty, question_result):
		...
		# Set-up Katakana question space
		...
		# Place them in the Katakana frame
		...
		with open(styles.get_char_set("./assets/character_keys/", "katakana.json"), "r", encoding='utf-8') as f:
			self.katakana = json.load(f)

		# Now the hard part: Translate all the Katakana characters to buttons
		...
		for j in range(0, 22):
			for i in range(0, 6):
				...
				if key_index > len(self.katakana):
					continue

				if difficulty < 3 and 46 <= key_index < len(self.katakana):
					state = tk.DISABLED
				else:
					state = tk.NORMAL

				tk.Button(self.frame_keyboard, text=self.katakana[key_index]["kana"], font=styles.FONT_KEYBOARD,
						  command=lambda o=key_index: self.key_pressed(self.katakana[o]), state=state).\
					grid(row=i + 1, column=j + 1, padx=[padv, 5], pady=5)

				key_index += 1

		tk.Button(self.frame_keyboard, text=self.katakana[key_index]["kana"], font=styles.FONT_KEYBOARD,
				  command=lambda o=key_index: self.key_pressed(self.katakana[o])).\
	...
	def start_question(self):
		...
		if self.difficulty < 3:
			...
			if index == 46:
				index = len(self.katakana) - 1
			self.answer = self.katakana[index]
		else:
			self.answer = self.katakana[random.randint(0, len(self.katakana) - 1)]
